...
from keras.layers import Conv2D, MaxPooling2D , AvgPool2D,AvgPool1D
...
def get_series_data( df, period, isNormaliztion, isBias) -> list:
    list_ = []
    ...
    if isBias : 
        ...
        for i in range(len(df)-period+1):
            if isNormaliztion :
                list_.append(normalize((df[i:i+period].values)*ratio, axis=0))
            else :
                list_.append((df[i:i+period].values)*ratio)
    else :
        ratio = 1000
        for i in range(len(df)-period+1):
            if isNormaliztion :
                list_.append(normalize((((df[i:i+period].values)/df.Close[i])-1)*ratio, axis=0))
            else :
                list_.append((((df[i:i+period].values)/df.Close[i])-1)*ratio) 
...
def get_turning_point(df ,TAIEX_df, length) :
    data = df.Close[::-1].reset_index(drop=True)
    start = data[0]
    if (sum(data[:math.ceil(length/2)])/(length/2))>start :
        isUP = True
    else :
        isUP = False
    ...
    for i in range(1,len(data)):
        ...
        if (i-count) > length :
            turning_date.append([TAIEX_df.index[count],TAIEX_df.Date[count],TAIEX_df.Close[count]])
            if isUP :
                isUP = False
            else:
                isUP = True
    return pd.Series(turning_date).drop_duplicates()
